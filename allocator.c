...
int init_mem() {
    ...
    __asan_poison_memory_region(first_node + 1, page_size);
    return 1;
}
...
void* my_malloc(const size_t size) {
    ...
    struct metadata *next_node = (void*)mdata + size + sizeof(struct metadata);
...
void my_free(void *ptr) {
    ...
    mdata->is_free = 1;
    __asan_poison_memory_region(mdata, mdata->size + sizeof(struct metadata));
}